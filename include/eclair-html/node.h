...
public:
  ...
  static Ptr newDocument() { return Ptr(new Node()); }

  static Ptr newElement(const TagName& name) { return Ptr(new Node(name)); }
  ...
  NodeKinds kind() const { return _kind; }

  const TagName& name() const { return _name; }

  Node* parent() const { return _parent; }

  const std::u16string& content() const { return _content; }
  ...
  ConstChildrenIt begin() const { return _children.begin(); }

  ConstChildrenIt end() const { return _children.end(); }

  Node& front() const { return *_children.front(); }

  Node& back() const { return *_children.back(); }
...
private:
  explicit Node() : _kind(NodeKinds::DOCUMENT), _parent(nullptr) {}
