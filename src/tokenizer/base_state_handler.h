...
namespace html_parser {
  ...
  public:
    BaseStateHandler(TokenReceiver& tokenReceiver,
        const std::u16string& data, TokenizerConfig& config,
        ErrorManager& errorManager)
      : _tokenReceiver(tokenReceiver), _config(config),
        _errorManager(errorManager), _eofValue(false),
        _stateValue(TokenizerStates::DATA),
        _returnState(TokenizerStates::DATA), _begin(data.begin()),
        _end(data.end()), _iterator(data.begin()), _startTag(true),
        _selfClosing(false), _docTypeInfo(new DocTypeInfo()),
        _referenceCode(0) {
  ...
  protected:
    ...
    inline bool _eof() const {
      return _eofValue;
    }

    inline char16_t _cc() const {
      return _ccValue;
    }

    inline TokenizerStates _state() const {
      return _stateValue;
    }
    ...
    bool _isAsciiAlphaNum() {
      return _isAsciiAlpha() || _isAsciiDigit();
    }
    ...
    bool _isTempBuffer(const char16_t* content) {
      return _tempBuffer == content;
    }
    ...
    bool _isHTMLNamespace() {
      return _config.isHTMLNamespace();
    }
    ...
    void _switchReturnState() {
      _switch(_returnState);
    }

    void _saveReturnState() {
      _returnState = _stateValue;
    }
    ...
    void _appendTempBufferLower() {
      _tempBuffer += Tools::toLowercase(_cc());
    }

    void _appendTagName() {
      _genericBuffer += Tools::toLowercase(_cc());
    }
    ...
    void _appendAttributeValue() {
      _attributeValueBuffer += _cc();
    }
    ...
    void _appendReplacementComment() {
      _genericBuffer += REPLACEMENT_CHAR;
    }

    void _appendDoctypeName() {
      _docTypeInfo->name += Tools::toLowercase(_cc());
    }
  ...
  private:
    ...
    void _pushAttribute() {
      ...
      auto insertRet = _attributes.insert(std::make_pair(_attributeNameBuffer,
            _attributeValueBuffer));
      if (!insertRet.second) {
