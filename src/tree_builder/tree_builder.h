...
public:
  explicit TreeBuilder(Node& document, ErrorManager& errorManager)
    ...
    _setMode<InsertionModes::AFTER_AFTER_FRAMESET, steps::AfterAfterFrameset>();
  }
  virtual ~TreeBuilder() {}
  ...
  TokenizerConfig& tokenizerConfig() { return state; }
...
private:
  ...
  bool _isHtmlContent(Token& token) {
    ...
    bool isMathML =
        steps::ForeignContent::isMathMLIntegrationPoint(currentNode);
    ...
    if (currentNode.name() == TagKinds::MATHML_ANNOTATION_XML &&
        token.kind == TokenKinds::START_TAG && token.name.value() == u"svg") {
      return true;
    }
  ...
  template <InsertionModes MODE, class TYPE> inline void _setMode() {
