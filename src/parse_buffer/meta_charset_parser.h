      ...
      if (_data[_position] == '<') {
        ...
        if (_isSequence("meta", true)) {
          if (_position + 5 < _length &&
              (_data[_position + 5] == '/' || _isSpace(_data[_position + 5]))) {
            _position += 5;
...
private:
  ...
  bool _isSequence(const char* sequence, bool caseSensitive) {
    std::size_t i = 0;
    for (; _position + i + 1 < _length && sequence[i] != '\0'; i++) {
      if (caseSensitive) {
  ...
  _ParseResults _parseAttributeName(std::string& name) {
    for (; _position < _length &&
           (_isSpace(_data[_position]) || _data[_position] == '/');
         _position++) {
    }
  ...
  std::string _parseMetaAttributes() {
    ...
    if (charset == "utf-16" || charset == "utf-16be" || charset == "utf-16le") {
      charset = "utf-8";
  ...
  std::string _fromContent(const std::string& input) {
    ...
    while (pos < input.size()) {
      ...
      while (pos < input.size() && !_isSpace(input[pos] && input[pos] != ';')) {
        res += input[pos];
        pos++;
      }
      return res;
