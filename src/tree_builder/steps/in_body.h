...
public:
  explicit InBody(RootBuilder& rootBuilder) : BaseStep(rootBuilder) {}
...
private:
  ...
  void _endTag(Token& token) {
    ...
    case TagKinds::H5:
    case TagKinds::H6:
      if (!_root.openElements.hasElementsInScope(
              ScopeKinds::STANDARD,
      ...
      _root.openElements.popUntil({TagKinds::H1, TagKinds::H2, TagKinds::H3,
                                   TagKinds::H4, TagKinds::H5, TagKinds::H6});
      break;
  ...
  void _handleStartForm(Token& token) {
    ...
    if (_root.openElements.hasElementInScope(ScopeKinds::BUTTON, TagKinds::P)) {
      _closeP(token);
    }
  ...
  void _handleStartA(Token& token) {
    auto badElement =
        _root.activeFormattingElements.findUntilMarker(TagKinds::A);
    if (badElement) {
      _root.errorManager.add(ErrorKinds::UNEXPECTED_TAG, token);
      _root.adoptionAgency.run(token);
  ...
  bool _hasOpenedImportantTags() {
    for (auto it = _root.openElements.begin(); it != _root.openElements.end();
         ++it) {
      switch ((*it)->name().kind()) {
      case TagKinds::DD:
      case TagKinds::DT:
  ...
  void _handleEndForm(Token& token) {
    if (_root.openElements.contains(TagKinds::TEMPLATE)) {
      ...
      if (!_root.state.form || !_root.openElements.hasElementInScope(
                                   ScopeKinds::STANDARD, *_root.state.form)) {
