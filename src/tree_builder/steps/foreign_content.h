...
public:
  explicit ForeignContent(RootBuilder& rootBuilder) : BaseStep(rootBuilder) {}
...
private:
  ...
  void _handleAnyStart(Token& token) {
    static const std::unordered_map<std::u16string, std::u16string> NAME_MAP = {
  ...
  void _handleEndScript(Token&) { _root.openElements.pop(); }
