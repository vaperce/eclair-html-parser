...
public:
  ...
  bool make(const char* charset) { return _convertData(charset); }
  ...
  bool make(const char* url, const char* httpCharset) {
    ...
    Encoding encoding = CompactEncDet::DetectEncoding(
        _data, _length, url, httpCharset, metaCharset.c_str(), UNKNOWN_ENCODING,
        UNKNOWN_LANGUAGE, CompactEncDet::WEB_CORPUS, true, &bytesConsumed,
        &isReliable);
...
private:
  inline void _log(const char* url, const char* httpCharset,
                   const std::string& metaCharset, Encoding encoding,
                   bool reliable) {
  ...
  bool _convertData(const char* charset) {
    ...
    while (true) {
      UChar32 code = ucnv_getNextUChar(conv, &sourcePtr, sourceLimit, &status);
      if (status == U_ZERO_ERROR) {
        _addToContent(code);
      } else {
        break;
