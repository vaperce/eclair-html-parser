  ...
  public:
    InsertionMode(const OpenElements& openElements, const State& state)
      : _openElements(openElements), _state(state),
        _mode(InsertionModes::INITIAL),
        _originalMode(InsertionModes::INITIAL) {
    }

    InsertionModes get() const {
      return _mode;
    }

    void set(InsertionModes mode) {
      if (mode == InsertionModes::TEXT ||
          mode == InsertionModes::IN_TABLE_TEXT) {
        _originalMode = _mode;
      }
      _set(mode);
    }
    ...
    void reset() {
      for (auto element = _openElements.cbegin();
          element != _openElements.cend(); ++element) {
    ...
    InsertionModes getTemplate() const {
      return _templateModes.back();
    }

    void pushTemplate(InsertionModes mode) {
      _templateModes.push_back(mode);
    }
    ...
    bool hasTemplates() {
      return !_templateModes.empty();
    }

  private:
    inline void _set(InsertionModes mode) {
      _mode = mode;
    }
