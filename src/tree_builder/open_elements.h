...
public:
  ...
  void push(Node& element) { _elements.push_front(&element); }

  void insert(It where, Node& element) { _elements.insert(where, &element); }

  Node& bottom() const { return *_elements.back(); }
  ...
  Node& top() const { return *_elements.front(); }
  ...
  CIt cbegin() const { return _elements.cbegin(); }

  CIt cend() const { return _elements.cend(); }

  It begin() { return _elements.begin(); }

  It end() { return _elements.end(); }

  It find(const Node& element) {
    for (auto openElement = _elements.begin(); openElement != _elements.end();
         ++openElement) {
  ...
  void popUntil(const std::vector<TagKinds>& kinds, bool included = true) {
    while (!_elements.empty()) {
      for (auto kind : kinds) {
        if (top().name() == kind) {
          if (included) {
  ...
  void popUntil(It it) { _elements.erase(_elements.begin(), std::next(it)); }
  ...
  It erase(It it) { return _elements.erase(it); }
