...
public:
  ...
  void insertElement(TagKinds kind) { _insertElement(kind, _target()); }
  ...
  void insertCharacter(const Token& token) { _insertCharacter(token.data); }

  void insertReplacementCharacter() { _insertCharacter(u"\uFFFD"); }

  void enableFosterParenting() { _fosterParenting = true; }

  void disableFosterParenting() { _fosterParenting = false; }
...
private:
  ...
  Node& _target(Node* overrideTarget = nullptr) {
    ...
    if (_fosterParenting &&
        (res.name() == TagKinds::TABLE || res.name() == TagKinds::TBODY ||
         res.name() == TagKinds::TFOOT || res.name() == TagKinds::THEAD ||
         res.name() == TagKinds::TR)) {
